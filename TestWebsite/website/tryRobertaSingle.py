...
tryRobertaSingle = Blueprint("tryRobertaSingle", __name__)


@tryRobertaSingle.route("/tryRobertaSingle")
def tryModel():
    return render_template("tryRobertaSingle.html")


@tryRobertaSingle.route("/tryRobertaSingle/get", methods=["GET", "POST"])
def chat():
    msg = request.form["msg"]
    param_index = int(request.form["param_index"])
    bit_num = int(request.form["bit_num"])
    input = msg
    return RobertaSingleInjector(input, param_index=param_index, num_bits=bit_num)


def RobertaSingleInjector(input_text, param_index=1, num_bits=1):
