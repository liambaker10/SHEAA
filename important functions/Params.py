...
def get_parameter_value_by_index(model, parameter_index):
    ...
    if isinstance(model, dict):
        raise ValueError(
            "The model should not be a dictionary when retrieving parameter by index."
        )
    else:
...
def get_parameter_value_by_index_modified(model, parameter_index):
    ...
    if isinstance(model, dict):
        raise ValueError(
            "The model should not be a dictionary when retrieving a parameter by index."
        )
    else:
        model_parameters = list(model.parameters())
        if parameter_index < len(model_parameters):
...
def get_parameter_values(model):
    ...
    if isinstance(model, dict):
        raise ValueError(
            "The model should not be a dictionary when retrieving parameter values."
        )
    else:
        model_parameters = list(model.parameters())
...
print(
    "Value of parameter at index {} in RoBERTa-base is:".format(parameter_index_roberta)
)
print(value_roberta)
