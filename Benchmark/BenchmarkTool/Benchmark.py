"""Benchmark implemented as a package that can be imported"""
import os
import datetime
import signal
import torch
...
class Benchmark:
    ...
    def handle_interrupt(self, signum, frame):
        print("\nBenchmark interrupted. Saving results and exiting...")
    ...
    def generate_summaries(self):
        ...
        input_encodings = {
            key: val.to(self.device) for key, val in input_encodings.items()
        }
        ...
        for i in range(0, len(input_encodings["input_ids"]), self.batch_size):
            ...
            generated_summaries = np.concatenate(
                (generated_summaries, batch_summaries), axis=None
            )
    ...
    def calculate_rouge_scores(self):
        ...
        rouge_scores = np.empty(
            (len(self.generated_summaries), 2), dtype=object
        )  # 2 columns for 'ROUGE-1', 'ROUGE-2'

        for idx, (generated_summary, target_summary) in enumerate(
            zip(self.generated_summaries, self.dataset.target_texts)
        ):
